  ...
  <div class="ratingselect">
    <div class="rating-type border-1px">
      ...
      <span class="block positive" :class="{'active':selectType===0}"
            @click="select(0,$event)">{{desc.positive}}<span class="count">{{positive.length}}</span></span>
      <span class="block negative" :class="{'active':selectType===1}"
            @click="select(1,$event)">{{desc.negative}}<span class="count">{{negative.length}}</span></span>
    </div>
    ...
    export default {
      ...
      methods: {
        select (type, event) {
          ...
          this.$emit('CStype', type);
        },
        toggleContent (event) {
          if (!event._constructed) {
            return;
          }
          this.$emit('CSonlytype');
        }
      },
      computed: {
        positive () {
